...
function renderWelcome() {
    ...
    gameEl.innerHTML = `
    <div class='welcome'>
    <button id='level0'>天堂<tton>
    <button id='advance'>普通<tton>
    <button id='advance1'>地狱<tton>
    </div>
    `;
    let buttonEl = gameEl.querySelector('button#level0');
    buttonEl.onclick = () => {
        let game = new Game(20, 15, 0.15);
        game.shuffle();
        renderTable(game);
    ...
    buttonEl.onclick = () => {
        let game = new Game(30, 20, 0.16);
        game.shuffle();
        renderTable(game);
    }
    buttonEl = gameEl.querySelector('button#advance1');
    buttonEl.onclick = () => {
        let game = new Game(40, 40, 0.18);
        game.shuffle();
        renderTable(game);
    }
}
